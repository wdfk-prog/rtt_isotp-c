...
static void server_thread_entry(void *param)
{
    ...
    while (1)
    {
        ...
        if (result == RT_EOK)
        {
            ...
            _print_hex_data("[Server] Sending Response", rx_payload, received_size);
        }
    }
}
...
static void logger_thread_entry(void *param)
{
    ...
    while (1)
    {
        ...
        {
            _print_hex_data("[Logger] Logged Command", logger_recv_buf, received_size);
        }
    }
}
...
static void client_thread_entry(void *param)
{
    ...
    while (1)
    {
        ...
        _print_hex_data("[Client] Sending Command", request_payload, sizeof(request_payload));
        ...
        if (isotp_rtt_send(client_link, request_payload, sizeof(request_payload), (200 * RT_TICK_PER_SECOND / 1000)) == RT_EOK)
        {
            ...
            if (result == RT_EOK)
            {
                _print_hex_data("[Client] Received Response", response_buf, response_size);
